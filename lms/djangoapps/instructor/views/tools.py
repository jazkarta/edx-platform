...
def dump_student_extensions(course, student):
    ...
    units = dict([(str(u.location), u) for u in units])
    query = StudentFieldOverride.objects.filter(
        course_id=course.id,
        student=student,
        field='due')
    for override in query:
        # Note that the reason we convert to a string here is because locators
        # don't currently round trip properly(?) with
        # xmodule_django.models.LocationKeyField, which uses str(locator) to
        # save the locator in the location column.  Unfortunately str(locator)
        # loses the course run, included in the locations of units above, and
        # also in block.location when saving the override in the first place.
        # Unfortunately, block_new.location != block_saved.location.
        # Converting to string is kind of a hacky work around that works for
        # now.
        location = str(override.location)
